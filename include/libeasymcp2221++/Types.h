...
enum class Pin : std::uint8_t { GP0 = 0, GP1 = 1, GP2 = 2, GP3 = 3 };

/** @brief High-level MCP2221 pin functions. */
enum class PinFunction { Dedicated, Alt0, Alt1, Alt2, GpioInput, GpioOutput };

/** @brief GPIO direction selector used by SRAM configuration. */
enum class GpioDirection { Output, Input };
 ...
 */
enum class SramPinFunction { Gpio, Dedicated, Alt0, Alt1, Alt2 };

/** @brief I2C transfer sequencing mode. */
enum class I2cTransfer { Normal, RepeatedStart, NoStop };

/** @brief Register-address width used by I2C target helpers. */
enum class RegisterWidth : std::uint8_t { Bits8 = 1, Bits16 = 2, Bits24 = 3, Bits32 = 4 };
...
enum class VoltageReference { Off, Vdd, Internal1V024, Internal2V048, Internal4V096 };

/** @brief Interrupt-on-change edge selection. */
enum class InterruptEdge { None, Rising, Falling, Both };

/** @brief GPIO edge-event type. */
enum class GpioEdge { Rising, Falling };

/** @brief Supported clock-output frequencies. */
enum class ClockFrequency { KHz375, KHz750, MHz1_5, MHz3, MHz6, MHz12, MHz24 };

/** @brief Supported clock-output duty cycles. */
enum class ClockDutyCycle { Percent0, Percent25, Percent50, Percent75 };

/** @brief Public persistent flash section identifiers. */
enum class FlashSection { ChipSettings, GpSettings, UsbManufacturer, UsbProduct, UsbSerial, ChipSerial };
