...
class LIBEASYMCP2221_CPP_API Device {
  public:
	 ...
	 * @return Stateful I2C target adapter.
	 */
	[[nodiscard]] I2cDevice i2cDevice(std::uint8_t address, const I2cDeviceOptions& options);
	 ...
	 */
	void i2cWrite(std::uint8_t address, const std::uint8_t* data, std::size_t size,
	 ...
	 * @param transfer Transfer mode.
	 */
	void i2cWrite(std::uint8_t address, const std::vector<std::uint8_t>& data,
				  I2cTransfer transfer = I2cTransfer::Normal);
	 ...
	 */
	[[nodiscard]] std::vector<std::uint8_t> i2cRead(std::uint8_t address, std::size_t size,
													I2cTransfer transfer = I2cTransfer::Normal);
	 ...
	 */
	[[nodiscard]] std::array<std::uint8_t, 64> rawCommand(const std::uint8_t* command, std::size_t size);
