...
GpioMonitor::interval() const noexcept
{
...
void GpioMonitor::setInterval(
    std::chrono::milliseconds interval)
{
    if (interval.count() <= 0) {
        throw std::invalid_argument(
            "GPIO polling interval must be positive");
    }

    if (interval.count() >
        std::numeric_limits<int>::max()) {
        throw std::invalid_argument(
            "GPIO polling interval is too large");
    }
...
std::size_t
GpioMonitor::maxEventsPerPoll() const noexcept
{
    return maxEventsPerPoll_;
}

void GpioMonitor::setMaxEventsPerPoll(
    std::size_t maxEvents)
{
    maxEventsPerPoll_ = maxEvents;
}

void GpioMonitor::setFilter(
    const GpioEventFilter& filter)
{
...
void GpioMonitor::pollOnce()
{
    try {
        const auto events =
            poller_.pollEvents(maxEventsPerPoll_);
