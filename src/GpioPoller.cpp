...
{
    switch (gpio) {
    case 0: return Pin::GP0;
    case 1: return Pin::GP1;
    case 2: return Pin::GP2;
    case 3: return Pin::GP3;
    }
...
{
    switch (type) {
    case MCP2221_GPIO_EVENT_RISE: return GpioEdge::Rising;
    case MCP2221_GPIO_EVENT_FALL: return GpioEdge::Falling;
    }
...
std::array<GpioChange, 4> GpioPoller::poll()
{
    ...
    for (std::size_t i = 0; i < result.size(); ++i) {
        result[i].oldValue =
            toOptionalGpioValue(native[i].old_value, "Polling GPIO changes");
        result[i].newValue =
            toOptionalGpioValue(native[i].new_value, "Polling GPIO changes");
...
std::vector<GpioEvent> GpioPoller::pollEvents(std::size_t maxEvents)
{
    ...
    if (count < 0) {
        detail::checkError(
            static_cast<mcp2221_error_code_t>(count),
            "Polling GPIO events");
    }
