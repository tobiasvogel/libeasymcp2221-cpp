...
Error::Error(ErrorCode code, int nativeCode, const std::string& message)
	: std::runtime_error(message), code_(code), nativeCode_(nativeCode) {}
...
ErrorCode toErrorCode(mcp2221_error_code_t code) {
	switch (code) {
		...
		case MCP2221_ERR_USB_INIT:
			return ErrorCode::UsbInit;
		case MCP2221_ERR_USB_ENUM:
			return ErrorCode::UsbEnumeration;
		case MCP2221_ERR_USB_OPEN:
			return ErrorCode::UsbOpen;
		case MCP2221_ERR_USB_CLAIM:
			return ErrorCode::UsbClaim;
...
[[noreturn]] void throwInvalid(const char* message) {
	throw Error(ErrorCode::Invalid, static_cast<int>(MCP2221_ERR_INVALID),
