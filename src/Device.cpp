...
mcp2221_gpio_pin_t toNativePin(Pin pin) {
	switch (pin) {
		case Pin::GP0:
			return MCP2221_GPIO_PIN_GP0;
		case Pin::GP1:
			return MCP2221_GPIO_PIN_GP1;
		case Pin::GP2:
			return MCP2221_GPIO_PIN_GP2;
		case Pin::GP3:
			return MCP2221_GPIO_PIN_GP3;
	}
...
mcp2221_pin_function_t toNativePinFunction(PinFunction function) {
	switch (function) {
		case PinFunction::Dedicated:
			return MCP2221_PIN_FUNC_DEDICATED;
		case PinFunction::Alt0:
			return MCP2221_PIN_FUNC_ALT0;
		case PinFunction::Alt1:
			return MCP2221_PIN_FUNC_ALT1;
		case PinFunction::Alt2:
			return MCP2221_PIN_FUNC_ALT2;
		case PinFunction::GpioInput:
			return MCP2221_PIN_FUNC_GPIO_IN;
		case PinFunction::GpioOutput:
			return MCP2221_PIN_FUNC_GPIO_OUT;
	}
...
int toNativeSramDirection(GpioDirection direction) {
	switch (direction) {
		case GpioDirection::Output:
			return MCP2221_DIR_OUTPUT;
		case GpioDirection::Input:
			return MCP2221_DIR_INPUT;
	}
...
int toNativeSramPinFunction(SramPinFunction function) {
	switch (function) {
		case SramPinFunction::Gpio:
			return MCP2221_GPIO_FUNC_GPIO;
		case SramPinFunction::Dedicated:
			return MCP2221_GPIO_FUNC_DEDICATED;
		case SramPinFunction::Alt0:
			return MCP2221_GPIO_FUNC_ALT_0;
		case SramPinFunction::Alt1:
			return MCP2221_GPIO_FUNC_ALT_1;
		case SramPinFunction::Alt2:
			return MCP2221_GPIO_FUNC_ALT_2;
	}

	detail::throwInvalid("Unknown SRAM pin function");
}

void applyAdcReference(VoltageReference reference, mcp2221_sram_adc_config_t& native) {
	switch (reference) {
...
void applyDacReference(VoltageReference reference, mcp2221_sram_dac_ref_config_t& native) {
	switch (reference) {
		case VoltageReference::Off:
...
int toNativeClockFrequency(ClockFrequency frequency) {
	switch (frequency) {
		case ClockFrequency::KHz375:
			return MCP2221_CLK_FREQ_375kHz;
		case ClockFrequency::KHz750:
			return MCP2221_CLK_FREQ_750kHz;
		case ClockFrequency::MHz1_5:
			return MCP2221_CLK_FREQ_1_5MHz;
		case ClockFrequency::MHz3:
			return MCP2221_CLK_FREQ_3MHz;
		case ClockFrequency::MHz6:
			return MCP2221_CLK_FREQ_6MHz;
		case ClockFrequency::MHz12:
			return MCP2221_CLK_FREQ_12MHz;
		case ClockFrequency::MHz24:
			return MCP2221_CLK_FREQ_24MHz;
	}
...
int toNativeClockDuty(ClockDutyCycle duty) {
	switch (duty) {
		case ClockDutyCycle::Percent0:
			return MCP2221_CLK_DUTY_0;
		case ClockDutyCycle::Percent25:
			return MCP2221_CLK_DUTY_25;
		case ClockDutyCycle::Percent50:
			return MCP2221_CLK_DUTY_50;
		case ClockDutyCycle::Percent75:
			return MCP2221_CLK_DUTY_75;
	}
...
Device::Device() : Device(DeviceOptions{}) {}

Device::Device(const DeviceOptions& options) {
	if (options.i2cSpeedHz == 0 || options.i2cSpeedHz > constants::MaxI2cSpeedHz) {
		detail::throwInvalid("Initial I2C speed must be from 1 through 400000 Hz");
	}

	mcp2221_t* handle = nullptr;
	const char* serial = options.usbSerial.empty() ? nullptr : options.usbSerial.c_str();

	detail::checkError(
		mcp2221_open_scan(options.vendorId, options.productId, options.deviceIndex, serial, options.usbReadTimeoutMs,
						  options.commandRetries, options.debugMessages ? 1 : 0, options.tracePackets ? 1 : 0,
						  options.scanFlashSerial ? 1 : 0, &handle),
		"Opening MCP2221");
...

	{
		...
		detail::checkError(mcp2221_i2c_set_speed(state->handle(), 100000), "Setting safe initial I2C speed");

		if (options.i2cSpeedHz != 100000) {
			detail::checkError(mcp2221_i2c_set_speed(state->handle(), options.i2cSpeedHz),
...
I2cDevice Device::i2cDevice(std::uint8_t address, const I2cDeviceOptions& options) {
	requireOpen(state_);
	validateI2cAddress(address);

	if (options.speedHz == 0 || options.speedHz > constants::MaxI2cSpeedHz) {
...
SmbusDevice Device::smbusDevice(std::uint8_t address) {
	...
	detail::checkError(mcp2221_smbus_init(&child->bus, state_->handle(), 0, 0, 0, nullptr, 0),
...
void Device::setI2cSpeed(std::uint32_t hz) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_i2c_set_speed(state_->handle(), hz), "Setting I2C speed");
}

void Device::i2cWrite(std::uint8_t address, const std::uint8_t* data, std::size_t size, I2cTransfer transfer) {
	requireOpen(state_);
	validateI2cAddress(address);
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_i2c_write_simple(state_->handle(), address, data, size, nativeTransfer),
					   "Writing I2C data");
}

void Device::i2cWrite(std::uint8_t address, const std::vector<std::uint8_t>& data, I2cTransfer transfer) {
	i2cWrite(address, data.data(), data.size(), transfer);
}

std::vector<std::uint8_t> Device::i2cRead(std::uint8_t address, std::size_t size, I2cTransfer transfer) {
	requireOpen(state_);
	validateI2cAddress(address);
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_i2c_read_simple(state_->handle(), address, data.data(), data.size(), nativeTransfer),
					   "Reading I2C data");
...
I2cStatus Device::i2cStatus() {
	...
	{
		std::lock_guard<std::mutex> lock(state_->mutex());
		detail::checkError(mcp2221_i2c_status(state_->handle(), &native), "Reading I2C status");
	}
...
void Device::releaseI2c() {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_i2c_release(state_->handle()), "Releasing I2C engine");
}
...
GpioState Device::readGpio() {
	...
	{
		std::lock_guard<std::mutex> lock(state_->mutex());
		detail::checkError(mcp2221_gpio_read_mask(state_->handle(), nativeState, &validMask), "Reading GPIO state");
	}
...
void Device::writeGpio(const GpioWrite& values) {
	...
	mcp2221_gpio_write_t request{MCP2221_GPIO_KEEP, MCP2221_GPIO_KEEP, MCP2221_GPIO_KEEP, MCP2221_GPIO_KEEP};

	int* nativeValues[4] = {&request.gp0, &request.gp1, &request.gp2, &request.gp3};
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_gpio_write(state_->handle(), &request), "Writing GPIO state");
}
...
void Device::setPinFunction(Pin pin, PinFunction function) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_pin_set_function(state_->handle(), toNativePin(pin), toNativePinFunction(function)),
...
void Device::configurePins(const PinConfigurations& configuration) {
	...
	for (std::size_t i = 0; i < configuration.size(); ++i) {
		...
		if (!entry.function.has_value()) {
			if (entry.outputValue) {
				detail::throwInvalid("outputValue must be false when a pin function is preserved");
			}
		...
		if (*entry.function == PinFunction::GpioOutput) {
			...
			if (entry.outputValue) {
				detail::throwInvalid("outputValue may be true only for PinFunction::GpioOutput");
			}
			native.out[i] = 0;
		}
	}
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_pin_set_functions(state_->handle(), &native), "Configuring GPIO pin functions");
}
...
void Device::setVdd(double volts) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_analog_set_vdd(state_->handle(), volts), "Setting VDD");
}
...
double Device::vdd() const {
	...
	double volts = 0.0;
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_analog_get_vdd(state_->handle(), &volts), "Reading VDD");
...
void Device::configureAdc(VoltageReference reference) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_adc_config(state_->handle(), toReferenceString(reference)), "Configuring ADC reference");
}
...
std::array<std::uint16_t, 3> Device::readAdcRaw() {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_adc_read_raw(state_->handle(), values.data()), "Reading raw ADC values");
...
std::array<double, 3> Device::readAdcNormalized() {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_adc_read_normalized(state_->handle(), values.data()), "Reading normalized ADC values");
...
std::array<double, 3> Device::readAdcVolts() {
	...
	std::array<double, 3> values{};
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_adc_read_volts(state_->handle(), values.data()), "Reading ADC voltages");
...
void Device::configureDac(VoltageReference reference) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_dac_config(state_->handle(), toReferenceString(reference)), "Configuring DAC reference");
}

void Device::configureDac(VoltageReference reference, std::uint8_t outputCode) {
	requireOpen(state_);
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_dac_config_out(state_->handle(), toReferenceString(reference), outputCode),
...
void Device::writeDacRaw(std::uint8_t code) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_dac_write_raw(state_->handle(), code), "Writing raw DAC output");
}
...
void Device::writeDacNormalized(double value) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_dac_write_normalized(state_->handle(), value), "Writing normalized DAC output");
}
...
void Device::writeDacVolts(double volts) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_dac_write_volts(state_->handle(), volts), "Writing DAC voltage");
}

void Device::configureClock(ClockDutyCycle duty, ClockFrequency frequency) {
	requireOpen(state_);

	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_clock_config(state_->handle(), toDutyPercent(duty), toClockFrequencyString(frequency)),
...
bool Device::interruptFlag() {
	...
	std::uint8_t flag = 0;
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_ioc_read(state_->handle(), &flag), "Reading interrupt-on-change flag");
...
void Device::clearInterruptFlag() {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_ioc_clear(state_->handle()), "Clearing interrupt-on-change flag");
}
...
void Device::configureInterrupt(InterruptEdge edge) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_ioc_config(state_->handle(), toInterruptEdgeString(edge)),
...
void Device::configureSram(const SramConfig& configuration) {
	...
	for (std::size_t i = 0; i < configuration.gpio.size(); ++i) {
		...
		native.gp[i].value = pin.outputValue.has_value() ? (*pin.outputValue ? 1 : 0) : MCP2221_CONFIG_KEEP;

		native.gp[i].direction =
			pin.direction.has_value() ? toNativeSramDirection(*pin.direction) : MCP2221_CONFIG_KEEP;

		native.gp[i].function = pin.function.has_value() ? toNativeSramPinFunction(*pin.function) : MCP2221_CONFIG_KEEP;
	}

	native.int_cfg.pos_edge = configuration.interrupt.risingEdge.has_value()
								  ? (*configuration.interrupt.risingEdge ? 1 : 0)
								  : MCP2221_CONFIG_KEEP;

	native.int_cfg.neg_edge = configuration.interrupt.fallingEdge.has_value()
								  ? (*configuration.interrupt.fallingEdge ? 1 : 0)
								  : MCP2221_CONFIG_KEEP;

	native.int_cfg.clear_flag = configuration.interrupt.clearFlag.has_value()
	...
	native.clk_cfg.div = configuration.clock.frequency.has_value()
							 ? toNativeClockFrequency(*configuration.clock.frequency)
							 : MCP2221_CONFIG_KEEP;

	native.clk_cfg.duty = configuration.clock.dutyCycle.has_value() ? toNativeClockDuty(*configuration.clock.dutyCycle)
																	: MCP2221_CONFIG_KEEP;

	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_sram_config(state_->handle(), &native), "Configuring runtime SRAM");
}
...
FlashData Device::readFlash(FlashSection section) {
	...
	FlashData data{};
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_flash_read(state_->handle(), toNativeFlashSection(section), data.data()),
					   "Reading flash section");

	return data;
}

void Device::writeFlash(FlashSection section, const FlashData& data) {
	requireOpen(state_);

	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_flash_write(state_->handle(), toNativeFlashSection(section), data.data()),
...
void Device::sendFlashPassword(const FlashPassword& password) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_flash_send_password(state_->handle(), password.data()), "Sending flash access password");
}
...
FlashInfo Device::flashInfo() {
	...
	{
		std::lock_guard<std::mutex> lock(state_->mutex());
		detail::checkError(mcp2221_flash_read_info(state_->handle(), &native), "Reading flash information");
	}

	FlashInfo info;
	std::copy(std::begin(native.chip_settings), std::end(native.chip_settings), info.chipSettings.begin());
	std::copy(std::begin(native.gp_settings), std::end(native.gp_settings), info.gpSettings.begin());
	std::copy(std::begin(native.usb_manufacturer), std::end(native.usb_manufacturer), info.usbManufacturerRaw.begin());
	std::copy(std::begin(native.usb_product), std::end(native.usb_product), info.usbProductRaw.begin());
	std::copy(std::begin(native.usb_serial), std::end(native.usb_serial), info.usbSerialRaw.begin());
	std::copy(std::begin(native.usb_factory_serial), std::end(native.usb_factory_serial),
...
FlashSettings Device::flashSettings() {
	...
	{
		std::lock_guard<std::mutex> lock(state_->mutex());
		detail::checkError(mcp2221_flash_get_settings(state_->handle(), &native), "Reading flash settings");
	}

	FlashSettings settings;
	std::copy(std::begin(native.chip_settings), std::end(native.chip_settings), settings.chip.begin());
	std::copy(std::begin(native.gp_settings), std::end(native.gp_settings), settings.gpio.begin());
...
void Device::saveConfigurationToFlash() {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_flash_save_config(state_->handle()), "Saving configuration to flash");
}
...
void Device::stageUsbRemoteWakeup(bool enabled) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_usb_set_remote_wakeup(state_->handle(), enabled ? 1 : 0), "Staging USB Remote Wake-up");
}
...
bool Device::usbRemoteWakeup() {
	...
	int enabled = 0;
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_usb_get_remote_wakeup(state_->handle(), &enabled), "Reading USB Remote Wake-up setting");
...
void Device::stageUsbSelfPowered(bool enabled) {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_usb_set_self_powered(state_->handle(), enabled ? 1 : 0),
...
bool Device::usbSelfPowered() {
	...
	int enabled = 0;
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_usb_get_self_powered(state_->handle(), &enabled), "Reading USB self-powered setting");
...
void Device::stageUsbRequestedCurrent(unsigned milliamps) {
	...
	if (milliamps > constants::MaxUsbCurrentMa || (milliamps % 2u) != 0u) {
		detail::throwInvalid("USB requested current must be an even value from 0 through 500 mA");
	}

	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_usb_set_requested_current(state_->handle(), milliamps), "Staging USB requested current");
}
...
unsigned Device::usbRequestedCurrent() {
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_usb_get_requested_current(state_->handle(), &milliamps),
					   "Reading USB requested current");

	return milliamps;
}

std::array<std::uint8_t, 64> Device::rawCommand(const std::uint8_t* command, std::size_t size) {
	requireOpen(state_);
	...
	std::lock_guard<std::mutex> lock(state_->mutex());
	detail::checkError(mcp2221_send_cmd(state_->handle(), command, size, response.data()),
